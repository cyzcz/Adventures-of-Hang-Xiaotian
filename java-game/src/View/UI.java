...
public class UI {
    ...
    private Image Levelimg = (new ImageIcon("src/image/UI/level.png")).getImage();
    ...
    public void draw(Graphics g) {
        if (visible){
            ...
            Font levelFont = new Font("宋体", Font.PLAIN, 20);
            //绘制HP图像
            ...
            g.drawString("" + (int) gf.actorData.getActorHPT()+ "/" + (int) gf.actorData.getActorHPLimit() , 51, 42);
            //绘制SP图像
            g.drawImage(SPimg, x + 39, y + 66, (int) (300 * (gf.actorData.getActorSP() / gf.actorData.getActorSPLimit())), 10, null);
            //绘制SP数值
            g.setFont(SPfont);
            g.setColor(Color.white);
            g.drawString("" + (int) gf.actorData.getActorSP() + "/" +(int) gf.actorData.getActorSPLimit() , 51, 75);
            g.drawImage(Levelimg, x + 46, y + 81, 181, 25, null);
            g.setFont(levelFont);
            g.setColor(Color.white);
            g.drawString("" + gf.actorData.getLevelName(gf.actorData.getLevel()), 51, 101);
